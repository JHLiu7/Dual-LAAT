...
logging.info(f"Downloading and preprocessing MIMIC-III data...")
...
discharge_summaries = prepare_discharge_summaries(mimic_notes.copy())
...
for col in [TARGET_COLUMN, "icd9_proc", "icd9_diag"]:
    full_dataset[f'full_{col}'] = full_dataset[col]
...
full_dataset = update_df_with_rare_codes(full_dataset, target_column=TARGET_COLUMN)


# print number of targets
logging.info(f"""
Number of full ICD-9 codes: {full_dataset['full_' + TARGET_COLUMN].explode().nunique()}
Number of frequent ICD-9 codes: {full_dataset[TARGET_COLUMN].explode().nunique()}
Number of rare ICD-9 codes: {full_dataset['rare_' + TARGET_COLUMN].explode().nunique()}
""")

# Preprocess the notes
full_dataset = preprocess_documents(full_dataset)
...
# Save the datasets
full_dataset.to_feather(output_dir / "mimiciii_icd9.feather")
logging.info(f"Saved full dataset to {output_dir / 'mimiciii_icd9.feather'}")
