...
DOWNLOAD_DIRECTORY_MIMICIII = "/scratch3/liu217/TmpICDs/repo/tmp_mimic_data"  # Path to the MIMIC-III data. Example: ~/physionet.org/files/mimiciii/1.4

DOWNLOAD_DIRECTORY_MIMICIV = "/scratch3/liu217/TmpICDs/repo/tmp_mimic_data"  # Path to the MIMIC-IV data. Example: ~/physionet.org/files/mimiciv/1.0
DOWNLOAD_DIRECTORY_MIMICIV_NOTE = "/scratch3/liu217/TmpICDs/repo/tmp_mimic_data"  # Path to the MIMIC-IV note data. Example: ~/physionet.org/files/mimiciv_note/1.0

DATA_DIRECTORY_PROCESSED = "/scratch3/liu217/TmpICDs/experiment_v5/data/coding_data"  # Path to the processed data. Example: ~/processed_data
...
def preprocess_documents(
    ...
    df = df.copy()
    df[f"RAW_{TEXT_COLUMN}"] = df[TEXT_COLUMN]
    ...
    df = preprocessor(df)
    df[f'NO_LOWERCASE_{TEXT_COLUMN}'] = df[TEXT_COLUMN]
...
def get_rare_target(full_target, target):
    """Get rare target codes by removing common target codes from full target codes.
    
    Args:
        full_target: Array or list of all target codes
        target: Array or list of common target codes to remove
        
    Returns:
        list: Rare target codes (full_target - target)
    """
    rare_codes =  [code for code in full_target if code not in set(target)]
    # drop nan if there are any
    return [code for code in rare_codes if code is not np.nan and code != "nan"]

def update_df_with_rare_codes(df, target_column=TARGET_COLUMN):
    """
    Updates dataframe with rare target codes and logs statistics.
    
    Args:
        df: DataFrame containing f'full_{target_column}' and target_column columns
        valid_codes: Set of valid codes to filter rare targets
        target_column: Name of the target column (default: TARGET_COLUMN)
        
    Returns:
        pd.DataFrame: Updated dataframe with f'rare_{target_column}' column
    """
    df = df.copy()
    
    # Create rare target column by filtering out common targets
    df[f'rare_{target_column}'] = df.apply(
        lambda row: get_rare_target(row[f'full_{target_column}'], row[target_column]), 
        axis=1
    )

    # Log comprehensive statistics
    total_rows = len(df)
    logging.info(f"Dataset contains {total_rows:,} rows")
    
    # Full target statistics
    num_full_target = df[f'full_{target_column}'].apply(len).sum()
    num_full_labels = df[f'full_{target_column}'].explode().nunique()
    logging.info(f"Full targets: {num_full_target:,} total codes ({num_full_labels:,} unique)")
    
    # Common target statistics  
    num_target = df[target_column].apply(len).sum()
    num_labels = df[target_column].explode().nunique()
    logging.info(f"Common {target_column}: {num_target:,} total codes ({num_labels:,} unique)")
    
    # Rare target statistics
    rows_with_rare = df[df[f'rare_{target_column}'].apply(len) > 0]
    num_rows_with_rare = len(rows_with_rare)
    pct_rows_with_rare = (num_rows_with_rare / total_rows) * 100
    logging.info(f"Rows with rare targets: {num_rows_with_rare:,} ({pct_rows_with_rare:.1f}%)")
    
    if num_rows_with_rare > 0:
        num_rare_target = rows_with_rare[f'rare_{target_column}'].apply(len).sum()
        num_rare_labels = rows_with_rare[f'rare_{target_column}'].explode().nunique()
        avg_rare_per_row = num_rare_target / num_rows_with_rare
        logging.info(f"Rare targets: {num_rare_target:,} total codes ({num_rare_labels:,} unique)")
        logging.info(f"Average rare targets per row with rare codes: {avg_rare_per_row:.1f}")
    else:
        logging.info("No rare targets found after filtering")
