...
class DualCNN(nn.Module):
    """CNN encoder + label-aware attention."""
...
class DualRNN(nn.Module):
    """BiLSTM/BiGRU encoder + label-aware attention."""
    ...
    def __init__(
        ...
        # Inter-layer dropout is only meaningful with >1 RNN layer; PyTorch warns otherwise.
        rnn_dropout = dropout if rnn_num_layers > 1 else 0.0
        ...
        self.rnn_text = rnn_cls(input_size=word_embeddings.shape[1], hidden_size=rnn_hidden_size, num_layers=rnn_num_layers, bidirectional=True, dropout=rnn_dropout, batch_first=True)
        self.rnn_code = rnn_cls(input_size=word_embeddings.shape[1], hidden_size=rnn_hidden_size, num_layers=rnn_num_layers, bidirectional=True, dropout=rnn_dropout, batch_first=True)
